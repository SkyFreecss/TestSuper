...
public class MergerReadExcel {
	 ...
	 * @param list
	 */
	public void readandwriteExcel(List<String> list,File filepathname)
	{
		try {
			WritableWorkbook wwb = Workbook.createWorkbook(new File(filepathname+"\\New_Test_"+date+".xls"));
			...
			WritableCellFormat format_s = new WritableCellFormat();
			format_s.setVerticalAlignment(VerticalAlignment.CENTRE);
			...
			for(int i=0;i<list.size();i++)
			{
				...
				oFirstSheet = wb.getSheet(0);//使用索引的形式获取第一个工作表。
					...
					for(int m = rowsNum_2;m<rows+rowsNum_2;m++)
					{
						for(int n=0;n<columns;n++)
						{
							...
							if(rowsNum_spc!=0)
							{
								...
								Label label1 = new Label(n,m,ocell1.getContents(),format_s);
								ws2.addCell(label1);
							}
						}
					}
				...
				else
				{
					int rows = oFirstSheet.getRows();//获取表的总行数
					int columns = oFirstSheet.getColumns();//获取表的总列数
					int[]newX=guoLv(rows, columns);
					rows=newX.length;
					int index=0;
					System.out.println(newX.length);
					for(int x = rowsNum;x<rows+rowsNum;x++)
					{
						for(int y = 0;y<columns;y++)
						{
							Cell ocell2 = oFirstSheet.getCell(y,newX[index]);//获取单元格内容
							Label label2 = new Label(y,x+1,ocell2.getContents(),format_s);//第一个参数的要写入的列数，第二个是写入行数，第三个是此坐标的内容
							ws.addCell(label2);
							num++;
...
/**
 *这个方法是用来找去掉表头和空行后的各行的位置
 * @author 满Buff的纯爷们儿
 * @param rows
 * @param columns
 * @return
 */
	public static int[] guoLv(int rows,int columns){
		int[]xPoint = new int[rows];
		int x=0;
		for(int i=0;i<rows;i++){
			if(i!=0){//去表头
				boolean b=true;
				for (int j = 0; j < columns; j++) {//去空行
					Cell ocell = oFirstSheet.getCell(j,i);
					if(ocell.getContents().equals("")){
						b=true;
					}else {
						b=false;
						break;
		...
		xPoint=Arrays.copyOf(xPoint,x);
//		System.out.println(Arrays.toString(xPoint));
		return xPoint;
	}
}
